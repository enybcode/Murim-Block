...
function corner(x,y,s=1,stroke=C.line) {
  return `<g transform="translate(${x} ${y}) scale(${s})" fill="none" stroke="${stroke}" stroke-width="2"><path d="M0 30V0h30M8 30V8h22"/><path d="M4 4l8 8" opacity=".45"/></g>`;
}
function frame(w,h,fill='url(#paper)',stroke=C.ink) {
  return `<rect x="10" y="10" width="${w-20}" height="${h-20}" rx="4" fill="${fill}" stroke="${stroke}" stroke-width="4"/><rect x="22" y="22" width="${w-44}" height="${h-44}" rx="2" fill="none" stroke="${stroke}" stroke-width="2" opacity=".82"/>${corner(22,22)}${corner(w-22,22,-1)}${corner(22,h-22,-1)}${corner(w-22,h-22,1)}`;
}
...
function qiIcon(color=C.qi) { return `<g transform="translate(48 48)" fill="none" stroke="${color}" stroke-width="7" stroke-linecap="round"><path d="M0-34C27-34 39-7 24 10 10 26-11 17-9 4 0 13 14 6 12-5 9-20-10-19-19-9"/><path d="M0 34C-27 34-39 7-24-10-10-26 11-17 9-4 0-13-14-6-12 5-9 20 10 19 19 9"/><circle r="5" fill="${color}" stroke="none"/></g>`; }
...
async function main(){
  ...
  await emit('Qi','qi_bar_background',600,64,`<path d="M20 4H580l16 28-16 28H20L4 32z" fill="${C.ink}" stroke="${C.line}" stroke-width="4"/><path d="M27 16h546l9 16-9 16H27l-9-16z" fill="#353a3d"/>`);
  await emit('Qi','qi_bar_fill',576,40,`<path d="M12 3h552l9 17-9 17H12L3 20z" fill="url(#blue)"/><path d="M23 9h526" stroke="#d4f9ff" stroke-width="4" opacity=".55"/>`);
  await emit('Qi','qi_bar_fill_gold',576,40,`<path d="M12 3h552l9 17-9 17H12L3 20z" fill="${C.gold}"/><path d="M23 9h526" stroke="#fff1bd" stroke-width="4" opacity=".55"/>`);
  ...
  await emit('Decorations','decor_corner',96,96,corner(10,10,2.4));
  ...
  const nav=(x,text,active=false)=>`<g transform="translate(${x} 625)"><path d="M8 2h232l10 10v55l-10 10H8L-2 67V12z" fill="${active?C.ink:'#f2ede2'}" stroke="${active?C.gold:C.line}" stroke-width="2"/>${sans(text,124,48,22,'middle',active?C.pale:C.ink,active?'600':'400')}</g>`;
  const profile=`${frame(1080,716)}<path d="M40 116H1040M40 610H1040" stroke="${C.line}" stroke-width="2"/>${label('MURIMBLOCK',540,78,31,'middle',C.ink,'600')}${separator(480,77)}${close}<g transform="translate(340 138)">${frame(400,320,'rgba(255,255,255,.18)',C.line)}<ellipse cx="200" cy="268" rx="95" ry="20" fill="none" stroke="${C.gold2}" stroke-width="3"/><path d="M200 86v120M140 206h120" stroke="${C.muted}" stroke-width="2" stroke-dasharray="8 8"/>${sans('ZONE JOUEUR 3D',200,168,18,'middle',C.muted,'600')}</g>${label('EnzoBertrand',540,486,28,'middle')}${sans('ROYAUME',540,515,15,'middle',C.muted,'600')}${label('Qi Refining III',540,545,23,'middle')}<g transform="translate(192 550) scale(.62)">${qiIcon()}</g><g transform="translate(256 558) scale(.72)"><path d="M20 4H580l16 28-16 28H20L4 32z" fill="${C.ink}" stroke="${C.line}" stroke-width="4"/><path d="M27 16h546l9 16-9 16H27l-9-16z" fill="#353a3d"/><path d="M27 18h382l7 14-7 14H27l-8-14z" fill="url(#blue)"/></g>${sans('725 / 1000',735,598,17,'start',C.ink)}${nav(30,'Profil',true)}${nav(293,'Techniques')}${nav(556,'Cultivation')}${nav(819,'Infos')}`;
